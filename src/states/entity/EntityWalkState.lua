EntityWalkState = Class{__includes = BaseState}

function EntityWalkState:init(entity, dungeon)
    self.entity = entity
    self.entity:changeAnimation('walk-down')

    self.dungeon = dungeon

    self.moveDuration = 0
    self.movementTimer = 0

    self.wallbumped = false
    self.objectbumped = false
end

function EntityWalkState:update(dt)
    
    self.wallbumped = false
    self.objectBump = false

    if self.dungeon ~= nil then
        for k, obj in ipairs(self.dungeon.currentRoom.objects) do
            if obj.type == 'pot' and self.entity:collides(obj) then
                self.objectBump = true
            end
        end
    end

    if self.entity.direction == 'left' then
        self.entity.x = self.entity.x - self.entity.walkSpeed * dt
        
        if self.entity.x <= MAP_RENDER_OFFSET_X + TILE_SIZE then 
            self.entity.x = MAP_RENDER_OFFSET_X + TILE_SIZE
            self.wallbumped = true
        end
    elseif self.entity.direction == 'right' then
        self.entity.x = self.entity.x + self.entity.walkSpeed * dt

        if self.entity.x + self.entity.width >= VIRTUAL_WIDTH - TILE_SIZE * 2 then
            self.entity.x = VIRTUAL_WIDTH - TILE_SIZE * 2 - self.entity.width
            self.wallbumped = true
        end
    elseif self.entity.direction == 'up' then
        self.entity.y = self.entity.y - self.entity.walkSpeed * dt

        if self.entity.y <= MAP_RENDER_OFFSET_Y + TILE_SIZE - self.entity.height / 2 then 
            self.entity.y = MAP_RENDER_OFFSET_Y + TILE_SIZE - self.entity.height / 2
            self.wallbumped = true
        end
    elseif self.entity.direction == 'down' then
        self.entity.y = self.entity.y + self.entity.walkSpeed * dt

        local bottomEdge = VIRTUAL_HEIGHT - (VIRTUAL_HEIGHT - MAP_HEIGHT * TILE_SIZE) 
            + MAP_RENDER_OFFSET_Y - TILE_SIZE

        if self.entity.y + self.entity.height >= bottomEdge then
            self.entity.y = bottomEdge - self.entity.height
            self.wallbumped = true
        end
    end
end

function EntityWalkState:processAI(params, dt)
    local room = params.room
    local player = room.player
    local directions = {}

    local playerX = player.x
    local playerY = player.y

    local entityX = self.entity.x
    local entityY = self.entity.y

    if playerX - entityX > 0 then
        if playerY - entityY > 0 then
            directions = {'right', 'down'}
        else
            directions = {'right', 'up'}
        end
    else
        if playerY - entityY > 0 then
            directions = {'left', 'down'}
        else
            directions = {'left', 'up'}
        end
    end

    if self.moveDuration == 0 then
        
        self.entity.direction = directions[math.random(#directions)]
        if self.entity.direction == 'left' or self.entity.direction == 'right' then
            self.moveDuration = math.random(math.abs(playerX - entityX)/ 3 + 1)
        else
            self.moveDuration = math.random(math.abs(playerY - entityY)/ 3 + 1)
        end
        self.entity:changeAnimation('walk-' .. tostring(self.entity.direction))
    elseif self.movementTimer > self.moveDuration then
        self.movementTimer = 0

        self.moveDuration = 0.5
        self.entity.direction = directions[math.random(#directions)]
        self.entity:changeAnimation('walk-' .. tostring(self.entity.direction))
    end

    self.movementTimer = self.movementTimer + dt
end

function EntityWalkState:render()
    local anim = self.entity.currentAnimation
    love.graphics.draw(gTextures[anim.texture], gFrames[anim.texture][anim:getCurrentFrame()],
        math.floor(self.entity.x - self.entity.offsetX), math.floor(self.entity.y - self.entity.offsetY))
end